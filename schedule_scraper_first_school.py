...
SECT_URL     = f"{MOBILE_BASE}/sectclassJSON?pid=1"
...
def fetch_schedule(sectids):
    body = "&".join([f"sect={s}" for s in sectids]) + "&newpatient=true"
...
def parse_lines(lines, sectname):
    ...
    for line in lines:
        ...
        records.append({
            "date":       date_str,
            "session":    parse_session(parts[1]),  # 改為讀取第 2 欄代碼 (A/P/N)
            "room":       parts[3].strip(),
...
def upsert_schedule(records, visit_type="初診"):
    conn   = get_conn()
    cursor = conn.cursor()
...
def main():
    ...
    inserted, skipped = upsert_schedule(all_records, visit_type="初診")
